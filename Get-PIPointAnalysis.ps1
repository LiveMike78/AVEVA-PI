# execute in PowerShell with the following parameters to generate CSV file
# ./Get-PIPointAnalysis.ps1  | Out-File -FilePath PIPointAnalysis.csv
...
# header for output, match names for PI builder compatibility
("Name;ObjectType;pointtype;archiving;future;pointsource;scan;creationdate;changedate;lastgoodvalue;lastgoodtimestamp;agedays;category")
...
foreach ($piPoint in $piPoints) {
    ...
    $creationDate = $piPoint.GetAttribute("creationdate")
    $changeDate = $piPoint.GetAttribute("changedate")
    ...
    if (($category -ne "Good") -or ($INCLUDE_GOOD -eq $true)) {
        ...
        ($piPoint.Name + ";PIPoint;" + $pointType + ";" + $archiving + ";" + $future + ";" + $pointSource + ";" + $scan + ";" + $creationDate + ";" + $changeDate + ";" + $value + ";" + $timestamp + ";" + $ageDays + ";" + $category)
    }
}